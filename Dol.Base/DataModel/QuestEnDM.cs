    ...
    public class QuestEnDM
    {
        public IList<Quest> Load()
        {
            using (MySqlConnection conn = new MySqlConnection("server = localhost; user id = root; password = ; database = test"))
            {
                ...
                cmd.ExecuteNonQuery();
                var cmdText = @"select id from quest_en";
        ...
        {
            var quest = new Quest();
            var query = "SELECT name,content,star,Discovery,DiscoveryType,DiscoveryLevel FROM quest_en WHERE id =" + id;
            var cmd = conn.CreateCommand();
            ...
            while (reader.Read())
            {
                ...
                quest.Discovery = reader.IsDBNull(3) ? null : reader.GetString(3);
                quest.DiscoveryType = reader.IsDBNull(4) ? null : reader.GetString(4);
                quest.DiscoveryLevel = reader.GetInt32(5);
            }
            reader.Close();
            query = "SELECT route_type,route,route_index FROM quest_routes_en  WHERE quest_id =" + id + " ORDER BY route_index";
            conn.CreateCommand();
            cmd.CommandText = query;
            ...
            reader.Close();
            query = "SELECT skill,level FROM quest_skills_en  WHERE quest_id =" + id;
            conn.CreateCommand();
            cmd.CommandText = query;
            reader = cmd.ExecuteReader();
            while (reader.Read())
